## 1.0.0
- 正式版(脚手架完成,首次合并进 main)。
- 全量累积:host_view_probe 沙盒模式/宿主可见性探测工具 + 宿主操作提权策略内嵌;
- system-prompt/assemble 自动注入(每轮 system prompt 常驻策略与实时沙盒模式,order=102);
- 修复链:schemastery 默认导入、plain JSON-Schema output、parameters 扁平化、依赖链接装配、defaultMode 回退。
